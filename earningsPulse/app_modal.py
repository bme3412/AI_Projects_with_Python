    ...
    if os.path.exists(transcript_dir):
        for file in os.listdir(transcript_dir):
            if file.endswith('.json'):
                ...
                parts = file.split('_')
                # Assuming the filename format is consistent with your example: AAPL_transcript_CY2020_Q1_2020-01-28.json
                # Creating a human-readable name
                readable_name = f"{parts[0]} {parts[3]} {parts[2]}"
                # Append both original and readable name
                transcript_files.append({'filename': file, 'readable_name': readable_name})
