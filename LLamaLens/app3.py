...
EXAMPLE_PHOTOS_FOLDER = 'static'
app.config['UPLOAD_FOLDER'] = UPLOAD_FOLDER
app.config['EXAMPLE_PHOTOS_FOLDER'] = EXAMPLE_PHOTOS_FOLDER
...
def upload_photo():
    if 'photo' in request.files and request.files['photo'].filename != '':
        ...
        if photo and allowed_file(photo.filename):
            ...
            session['photo_source'] = 'uploaded'
            return redirect(url_for('display_photo', filename=filename))

    elif 'example_photo' in request.form and request.form['example_photo'] != '':
        example_photo_url = request.form['example_photo']
        filename = example_photo_url.split('/')[-1]
        session['photo_source'] = 'example'
        return redirect(url_for('display_photo', filename=filename))
...
def display_photo(filename):
    photo_source = session.get('photo_source', 'uploaded')

    if photo_source == 'example':
        photo_path = os.path.join(EXAMPLE_PHOTOS_FOLDER, filename)
    else:
        photo_path = os.path.join(UPLOAD_FOLDER, filename)
    ...
    try:
        ...
        missing_columns = [col for col in columns_to_keep if col not in df.index]
        ...
        try:
            ...
            landmarks = [landmark['Name'] for landmark in response.get('Landmarks', [])]
        ...
        datestamp = df.loc['gps_datestamp', 'Value'] if 'gps_datestamp' in df.index else 'Unknown'
        lens_make = df.loc['lens_make', 'Value'] if 'lens_make' in df.index else 'Unknown'
        timestamp = datestamp.replace(':', '-') if datestamp != 'Unknown' else 'Unknown'
        ...
        return render_template('display_photo.html',
                               ...
                               rekognition_caption=rekognition_caption,
...
def example_photo(filename):
    return send_from_directory(app.config['EXAMPLE_PHOTOS_FOLDER'], filename)
