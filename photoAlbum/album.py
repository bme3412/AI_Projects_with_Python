...
table_name = 'bme3412travelphotos'
...
try:
    ...
    # Create the 'bme3412travelphotos' table if it doesn't exist
    table = dynamodb.create_table(
        TableName=table_name,
        KeySchema=[
            {
                'AttributeName': 'cotuit1429',
                'KeyType': 'HASH'
            }
        ],
...
def display_photo(filename):
    ...
    try:
        ...
        if not missing_columns:
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": f"Based on the location {location}, generate an interesting fact or informative phrase. Do not start with 'Additional Interesting Fact: ...'. Just state the fact "
                    }
                ],
                temperature=0.7,
                max_tokens=150,
        ...
        try:
            ...
            # Generate a brief caption based on the detected objects, scenes, landmarks, and location
            response = client.chat.completions.create(
                model="gpt-4-turbo",
                messages=[
                    {
                        "role": "system",
                        "content": "You are an AI assistant that generates brief captions for images based on detected objects, scenes, landmarks, and location."
                    },
                    {
                        "role": "user",
                        "content": f"Based on the following information, generate a brief caption for the image:\n\nDetected Objects and Scenes: {', '.join(labels)}\nDetected Landmarks: {', '.join(landmarks)}\nDetected Location: {location}"
                    }
                ],
                temperature=0.7,
                max_tokens=75,
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": f"Based on the following information, generate a brief and punny caption (no hashtags, but emojis are ok) suitable for a 36-year-old male Instagram user for the uploaded image:\n\nLocation: {location}\nInteresting Fact: {interesting_fact}\nDetected Objects: {', '.join(labels)}\nLandmarks: {', '.join(landmarks)}"
                    }
                ],
                temperature=0.7,
                max_tokens=150,
            ...
            response = client.chat.completions.create(
                ...
                messages=[
                    ...
                    {
                        "role": "user",
                        "content": f"Based on the following information, generate an additional interesting fact (but do not state it is an additional interesting fact) or informative phrase related to the uploaded image:\n\nLocation: {location}\nInteresting Fact: {interesting_fact}\nDetected Objects: {', '.join(labels)}\nLandmarks: {', '.join(landmarks)}"
                    }
                ],
                temperature=0.7,
                max_tokens=100,
...
def create_album():
    ...
    # Check if the album_id already exists in the database
    table = dynamodb.Table('bme3412travelphotos')
    response = table.get_item(Key={'album_id': album_id})
    if 'Item' in response:
        # Album ID already exists, handle the error or show a message to the user
        return "Album ID already exists. Please choose a different ID."
...
@app.route('/add_to_album/<path:filename>', methods=['POST'])
def add_to_album(filename):
    table = dynamodb.Table('bme3412travelphotos')
    response = table.scan()
    albums = response['Items']
    return render_template('add_to_album.html', filename=filename, albums=albums)


@app.route('/save_to_album/<path:filename>', methods=['POST'])
def save_to_album(filename):
    album_id = request.form.get('album_id')
    album_name = request.form.get('album_name')

    if not album_id and not album_name:
        # Handle the case when both 'album_id' and 'album_name' are missing
        return "Album ID or Album Name not provided", 400

    if not album_id:
        # Create a new album with the provided album name
        album_id = str(uuid.uuid4())  # Generate a unique ID for the album
    ...
    # Save the photo to the selected album using DynamoDB
    table = dynamodb.Table('bme3412travelphotos')
    table.put_item(
        Item={
            'filename': filename,
...
@app.route('/add_to_cloud/<path:filename>', methods=['POST'])
def add_to_cloud(filename):
    # Get the photo data
    photo_path = os.path.join(UPLOAD_FOLDER, filename)
    with open(photo_path, 'rb') as file:
        photo_data = file.read()

    # Upload the photo to the cloud (e.g., S3)
    s3 = boto3.client('s3')
    s3.put_object(Body=photo_data, Bucket='your-bucket-name', Key=filename)

    # Update the photo item in DynamoDB to include the cloud URL
    table = dynamodb.Table('bme3412travelphotos')
    table.update_item(
        Key={'filename': filename},
        UpdateExpression='SET cloud_url = :url',
        ExpressionAttributeValues={
            ':url': f"https://your-bucket-name.s3.amazonaws.com/{filename}"}
    )

    return redirect(url_for('view_album', album_id=request.form['album_id']))


@app.route('/view_album/<album_id>')
def view_album(album_id):
    # Retrieve the photos for the specified album from DynamoDB
    table = dynamodb.Table('bme3412travelphotos')
    ...
    return render_template('view_album.html', album_id=album_id, photos=photos)
